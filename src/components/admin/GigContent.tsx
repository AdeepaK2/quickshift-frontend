...
const mockGigs = [
  {
    ...
    description: "We need energetic staff to help with our annual summer festival.",
    payRate: { type: "hourly", min: 25, max: 30, currency: "$" },
    timeSlots: [
      { date: "2025-07-01", startTime: "09:00", endTime: "17:00", peopleNeeded: 10, peopleAssigned: 3 },
      { date: "2025-07-02", startTime: "09:00", endTime: "17:00", peopleNeeded: 10, peopleAssigned: 2 }
    ],
    location: {
    ...
    applicants: [
      { id: "app-001", name: "John Doe", status: "pending", appliedAt: "2025-05-20T14:30:00Z" },
      { id: "app-002", name: "Jane Smith", status: "accepted", appliedAt: "2025-05-21T10:15:00Z" }
    ],
  ...
  },
  {
    ...
    timeSlots: [
      { date: "2025-06-15", startTime: "10:00", endTime: "12:00", peopleNeeded: 2, peopleAssigned: 0 },
      { date: "2025-06-16", startTime: "10:00", endTime: "12:00", peopleNeeded: 3, peopleAssigned: 0 }
    ],
    location: {
  ...
  },
  {
    ...
    timeSlots: [
      { date: "2025-05-25", startTime: "08:00", endTime: "16:00", peopleNeeded: 10, peopleAssigned: 8 }
    ],
    location: {
    ...
    skills: ["Lifting up to 50lbs", "Basic computer skills", "Attention to detail"],
    ...
    isAcceptingApplications: true,
    applicants: [
      { id: "app-003", name: "Mike Johnson", status: "accepted", appliedAt: "2025-05-01T09:45:00Z" },
      { id: "app-004", name: "Sarah Williams", status: "rejected", appliedAt: "2025-05-02T14:20:00Z" },
      { id: "app-005", name: "David Brown", status: "accepted", appliedAt: "2025-05-03T11:10:00Z" }
    ],
...
const statusBadgeVariants: Record<string, "warning" | "success" | "info" | "default" | "destructive" | "outline" | "secondary"> = {
  draft: "warning",
  open: "success",
  in_progress: "info",
  completed: "default",
...
export default function GigContent() {
  ...
  const filteredGigs = mockGigs.filter(gig => {
    ...
    if (filters.employer && !gig.employer.name.toLowerCase().includes(filters.employer.toLowerCase())) return false;
    if (filters.city && !gig.city.toLowerCase().includes(filters.city.toLowerCase())) return false;
    if (filters.category && !gig.category.toLowerCase().includes(filters.category.toLowerCase())) return false;
  ...
  const uniqueEmployers = Array.from(new Set(mockGigs.map(g => g.employer.name)));
  ...
  const formatPayRate = (payRate: any) => {
    if (payRate.type === "fixed") {
      ...
      return `${payRate.currency}${payRate.min}${payRate.max !== payRate.min ? ` - ${payRate.currency}${payRate.max}` : ""} / ${payRate.type}`;
    }
  };
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {!gig.isAcceptingApplications && (
                        <div className="mt-1 flex items-center">
                          ...
                          <span className="text-xs text-amber-500">Not accepting applications</span>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{gig.employer.name}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{gig.category}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge variant={statusBadgeVariants[gig.status] || "default"}>
                      ...
                      <div className="flex space-x-2">
                            ...
                            {selectedGig && (
                              <div className="space-y-6">
                                <div className="border-b pb-4">
                                  ...
                                  <Badge variant={statusBadgeVariants[selectedGig.status] || "default"} className="mt-2">
                                  ...
                                  <p className="mt-4 text-gray-700">{selectedGig.description}</p>
                                </div>
                                ...
                                <div className="space-y-4">
                                  <div className="flex flex-col sm:flex-row sm:justify-between gap-4">
                                    <div>
                                      ...
                                      <p className="text-base">{selectedGig.employer.name}</p>
                                      <p className="text-sm text-gray-500">{selectedGig.employer.email}</p>
                                    </div>
                                    <div>
                                      <h3 className="text-sm font-medium text-gray-500">Category & Pay</h3>
                                      <p className="text-base">{selectedGig.category}</p>
                                      <p className="text-sm text-gray-500">{formatPayRate(selectedGig.payRate)}</p>
                                    </div>
                                  </div>
                                      ...
                                      <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gray-50">
                                          <tr>
                                            ...
                                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">People Needed</th>
                                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">People Assigned</th>
                                          </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-200">
                                          {selectedGig.timeSlots.map((slot: any, index: number) => (
                                            <tr key={index}>
                                              ...
                                              <td className="px-4 py-2 text-sm">{slot.peopleNeeded}</td>
                                              <td className="px-4 py-2 text-sm">{slot.peopleAssigned}</td>
                                            </tr>
                                          ))}
                                        </tbody>
                                      </table>
                                  ...
                                  <div>
                                    ...
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div>
                                        <p className="text-base">{selectedGig.location.address}</p>
                                  ...
                                  <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {selectedGig.skills.map((skill: string, index: number) => (
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div>
                                      ...
                                      <p className="text-sm">{selectedGig.experience}</p>
                                    </div>
                                    <div>
                                      <h3 className="text-sm font-medium text-gray-500">Dress Code</h3>
                                      <p className="text-sm">{selectedGig.dressCode}</p>
                                    </div>
                                    <div>
                                      <h3 className="text-sm font-medium text-gray-500">Equipment</h3>
                                      <p className="text-sm">{selectedGig.equipment}</p>
                                    </div>
                                  </div>
                                  ...
                                  <div>
                                    ...
                                    <p className="text-base">
                                      {new Date(selectedGig.applicationDeadline).toLocaleDateString()}
                                    </p>
                                  </div>

                                  <div>
                                    <h3 className="text-sm font-medium text-gray-500 mb-2">
                                      Positions: {selectedGig.filledPositions} filled out of {selectedGig.totalPositions}
                                    </h3>
                                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                                      <div 
                                        className="bg-blue-600 h-2.5 rounded-full" 
                                        style={{ width: `${(selectedGig.filledPositions / selectedGig.totalPositions) * 100}%` }}
                                      ></div>
                                    </div>
                                  </div>
                                  ...
                                  {selectedGig.applicants.length > 0 && (
                                    <div>
                                      <h3 className="text-sm font-medium text-gray-500 mb-2">Applicants ({selectedGig.applicants.length})</h3>
                                      <div className="border rounded-md overflow-hidden">
                                        <table className="min-w-full divide-y divide-gray-200">
                                          <thead className="bg-gray-50">
                                            <tr>
                                              ...
                                              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">Applied At</th>
                                            </tr>
                                          </thead>
                                          <tbody className="divide-y divide-gray-200">
                                            {selectedGig.applicants.map((applicant: any) => (
                                              <tr key={applicant.id}>
                                                <td className="px-4 py-2 text-sm">{applicant.name}</td>
                                                <td className="px-4 py-2 text-sm">
                                                  <Badge 
                                                    variant={
                                  ...
                                  <div className="grid grid-cols-2 gap-4 text-sm text-gray-500">
                                    <div>
                                      ...
                                      {new Date(selectedGig.createdAt).toLocaleString()}
                                    </div>
                                    <div>
                                      <span className="font-medium">Last Updated:</span>{" "}
                                      {new Date(selectedGig.updatedAt).toLocaleString()}
                                    </div>
                                  </div>
                                </div>
                              </div>
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="sm:max-w-md">
                            ...
                            <div className="space-y-4">
                              <p>Current status: <Badge variant={statusBadgeVariants[gig.status]}>{gig.status}</Badge></p>
                              ...
                              <div className="flex justify-end gap-2 mt-4">
                                ...
                                <Button 
                                  onClick={() => handleStatusChange(gig.id, "new-status")}
                                >
                                  Save Changes
                                </Button>
                              </div>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Showing <span className="font-medium">1</span> to <span className="font-medium">{filteredGigs.length}</span> of{" "}
