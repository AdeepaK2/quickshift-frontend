...
export async function testApiConnectivity(): Promise<boolean> {
  ...
  try {
    ...
    if (gigsResponse.data && gigsResponse.data.length > 0) {
      ...
      const gigResponse = await gigsApi.getById(firstGig._id);
      ...
      if (firstGig.status === "draft") {
        ...
        const statusResponse = await gigsApi.updateStatus(firstGig._id, "open");
        console.log(
          "✅ PATCH /api/gigs/:id/status successful:",
          statusResponse
        );

        // Revert status back
        await gigsApi.updateStatus(firstGig._id, "draft");
...
if (typeof window !== "undefined") {
  (window as unknown as { testApi: () => Promise<boolean> }).testApi =
    testApiConnectivity;
